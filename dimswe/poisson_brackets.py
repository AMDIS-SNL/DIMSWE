    ...
    def __init__(self, spaces, vars, parameters):
        ...
        if not spaces is None:
            if self.dim == 2:
                ...
                self.coriolis = Function(
                    spaces.CGV
                )  # PRETTY UNCLEAR ACTUALLY- MAYBE H(curl) or even H(div)?
        ...
        for name, bundle, degree in zip(
            ...
            if bundle == "S":
                rhs_expr = rhs_expr + SVLieDerivative(
                    degree, self.dim, u, qvars[name], xhats[name]
                )
                rhs_expr = rhs_expr - SVLieDerivative(
                    degree, self.dim, mtest, qvars[name], dfdx_vars[name]
                )
            elif bundle == "VV":
                rhs_expr = rhs_expr + VVLieDerivative(
                    degree, self.dim, u, qvars[name], xhats[name]
                )
                rhs_expr = rhs_expr - VVLieDerivative(
                    degree, self.dim, mtest, qvars[name], dfdx_vars[name]
                ...
                rhs_expr = rhs_expr + CVLieDerivative(
                    degree, self.dim, u, qvars[name], xhats[name]
                )
                rhs_expr = rhs_expr - CVLieDerivative(
                    degree, self.dim, mtest, qvars[name], dfdx_vars[name]
...
class CurlForm_AdvectedQuantities_Bracket(PoissonBracket):
    ...
    def compute_q_expressions(self, vars, expressions):
        if not self.upwind_v:
            ...
            if self.dim == 2:
                expressions["q"] = [
                    inner(qhat, total_dens * qtrial) * dx,
    ...
    def rhs(self, const_state, qvars, dfdx_vars, xhats):
        ...
        for name, bundle, degree in zip(
            ...
            if bundle == "S":
                rhs_expr = rhs_expr + SVLieDerivative(
                    degree, self.dim, F / total_dens, const_state[name], xhats[name]
                )
                rhs_expr = rhs_expr - SVLieDerivative(
                    degree,
                    self.dim,
...
class LiePoisson_AdvectedDensities_Bracket(PoissonBracket):
    ...
    def rhs(self, qvars, dfdx_vars, xhats):
        ...
        # FIX THIS STUFF UP!
        mtilde = 0.5 * ((1.0 + alpha) * m("+") + (1.0 - alpha) * m("-"))
        rhs_expr = (
            (mtest("+") * inner(u("+"), n("+")) + mtest("-") * inner(u("-"), n("-")))
            * mtilde
            * dS
        )
        rhs_expr = (
            rhs_expr
            - (u("+") * inner(mtest("+"), n("+")) + u("-") * inner(mtest("-"), n("-")))
            * mtilde
            * dS
        )
            ...
            if self.use_split_form[dens_name]:
                rhs_expr = (
                    ...
                    + inner(
                        ...
                        0.5
                        * (
                            div(dens / total_dens * F)
                            + dot(grad(dens / total_dens), F)
                            + dens / total_dens * div(F)
                        ),
                    )
                    * dx
                )
                rhs_expr = (
                    ...
                    + inner(
                        ...
                        0.5
                        * (
                            dens / total_dens * grad(Bdens)
                            + grad(Bdens * dens / total_dens)
                        ),
                    )
                    * dx
                    + 0.5 * inner(dens / total_dens, div(vtest * Bdens)) * dx
                )
            else:

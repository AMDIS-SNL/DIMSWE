...
class Output():
    def __init__(self, xn, coeff, parameters, dynamics, timestepper, logger):
        ...
        self.coefflist = self.dynamics.coefflist
    ...
    def output(self, t, step, output_step, stat_step):
        ...
        with CheckpointFile(self.chkpoint_name, 'a') as chkpoint_file:
            ...
            for i,var in enumerate(self.varlist):
                chkpoint_file.save_function(self.xn.sub(i), idx=output_step, name=var)
                self.vtk_vars.append(self.xn.sub(i))

            self.vtk_vars = []
            for i,var in enumerate(self.coefflist):
                chkpoint_file.save_function(self.coeff.sub(i), idx=output_step, name=var)
                self.vtk_vars.append(self.coeff.sub(i))

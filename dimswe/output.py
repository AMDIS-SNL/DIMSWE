...
class Output:
    ...
    def output(self, step, output_step, stat_step):
        self.logger.output(
            "output at step "
            + str(step)
            + " and output_step "
        ...
        with CheckpointFile(self.chkpoint_name, "a") as chkpoint_file:
            ...
            for i, var in enumerate(self.varlist):
                chkpoint_file.save_function(
                    self.timestepper.xn.sub(i), idx=output_step, name=var
                )

            if self.output_aux_vars:
                for var in self.q_aux_var_list:
                    chkpoint_file.save_function(
                        self.timestepper.q_aux_vars[var], idx=output_step, name=var
                    )

                for var in self.dfdx_aux_var_list:
                    chkpoint_file.save_function(
                        self.timestepper.dfdx_aux_vars[var], idx=output_step, name=var
                    )

            for var in self.dynamics.diagnostics.var_list:
                chkpoint_file.save_function(
                    self.dynamics.diagnostics.vars[var], idx=output_step, name=var
                )
            ...
            for statistic_name in self.dynamics.statistics.statistic_names:
                ...
                h5file.create_dataset(
                    statistic_name,
                    data=self.dynamics.statistics.statistics[statistic_name][
                        : stat_step + 1
                    ],
                )

...
class AdvDensStatistics:
    ...
    def create(self, xn):
        ...
        self.total_density_expression = (
            self.hamiltonian.vars.get_total_density_expr(xn) * dx
        )
    ...
    def compute(self, step, stat_step):
        self.statistics["total_energy"][stat_step] = assemble(
            self.total_energy_expression
        )
        self.statistics["total_density"][stat_step] = assemble(
            self.total_density_expression
        )
        for dens in self.density_names:
            self.statistics["total_" + dens][stat_step] = assemble(
                self.total_density_expressions[dens]
            )

...
def get_mesh_and_spaces(parameters, initcond):
    ...
    if parameters["mesh"] == "line":
        left = initcond.xc - initcond.Lx / 2.0
        right = initcond.xc + initcond.Lx / 2.0
    ...
    # NEED TO SCALE+CENTER MESH COORDINATES
        ...
        left_x = initcond.xc - initcond.Lx / 2.0
        right_x = initcond.xc + initcond.Lx / 2.0
        left_y = initcond.yc - initcond.Ly / 2.0
        right_y = initcond.yc + initcond.Ly / 2.0
        mesh = RectangleMesh(
            parameters["nx"],
            parameters["ny"],
            right_x,
            ...
            originX=left_x,
            originY=left_y,
            quadrilateral=not parameters["simplicial_cells"],
    ...
    # NEED TO SCALE+CENTER MESH COORDINATES
        ...
        mesh = BoxMesh(
            ...
            initcond.Ly,
            initcond.Lz,
            hexahedral=not parameters["simplicial_cells"],
            diagonal=parameters["diagonal"],
        )
    # NEED TO SCALE+CENTER MESH COORDINATES
    ...
    def __init__(self, mesh, parameters):
        ...
        if parameters["family"] == "Q":
            ...
            if mesh.cell_dimension() == 2:
                self.DGV = VectorFunctionSpace(
                    mesh, "DG", parameters["order"] - 1
                )  # TRY DQ?
                self.CGV = VectorFunctionSpace(
                    mesh, "CG", parameters["order"]
                )  # TRY Q?

from .variables import (
    ...
    ThermalShallowWaterVariables_CF_H1,
)
from .variables import (
    MoistThermalShallowWaterVariables_CF,
    MoistThermalShallowWaterVariables_LP,
    MoistThermalShallowWaterVariables_CF_H1,
)
from .variables import CompressibleEulerVariables_CF, CompressibleEulerVariables_LP
from .variables import (
    MaxwellVariables,
    MHDVariables_LP,
    EulerMaxwellVariables_LP,
    ScalarWaveVariables,
)
from .poisson_brackets import (
    LiePoisson_AdvectedDensities_Bracket,
    CurlForm_AdvectedDensities_Bracket,
    EulerMaxwellCouplingBracket_LP,
    MHDBracket_LP,
    ScalarWaveBracket,
...
from .metric_brackets import (
    ...
    ThermodynamicallyCompatibleViscousRegularization_LP,
)
from .hamiltonians import (
    CompressibleEuler_Hamiltonian_CF,
    CompressibleEuler_Hamiltonian_LP,
)
from .hamiltonians import (
    Maxwell_Hamiltonian,
    EulerMaxwell_Hamiltonian_LP,
    ScalarWave_Hamiltonian,
    MHD_Hamiltonian_LP,
    get_thermo,
)
from .hamiltonians import (
    ThermalShallowWater_Hamiltonian_CF,
    ThermalShallowWater_Hamiltonian_LP,
    ThermalShallowWater_Hamiltonian_CF_H1,
)
from .hamiltonians import (
    MoistThermalShallowWater_Hamiltonian_CF,
    MoistThermalShallowWater_Hamiltonian_LP,
    MoistThermalShallowWater_Hamiltonian_CF_H1,
)
from .entropies import SimpleEntropy
from .statistics import (
    AdvDensStatistics_CF,
    AdvDensStatistics_CF_H1,
    AdvDensStatistics_LP,
    MaxwellStatistics,
    MHDStatistics,
    EulerMaxwellStatistics,
    ScalarWaveStatistics,
)
from .diagnostics import (
    AdvDensDiagnostics_CF,
    AdvDensDiagnostics_CF_H1,
    AdvDensDiagnostics_LP,
    MaxwellDiagnostics,
    MHDDiagnostics,
...
class AdvectionDynamics(Dynamics):
    def __init__(self, mesh, spaces, initcond, vars, statistics, diagnostics, logger):
        ...
        self.form_degrees = parameters[
            "form_degrees"
        ]  # 0,1,2,3 in 3D, 0,1,2 in 2D (MAYBE NEED TO DISTINGUISH 1-FORMS AND N-1 FORMS HERE?)
    ...
    def rhs(self, qvars, dfdx_vars, xhats, t, terms="all"):
        ...
        if self.advection_type == "CF":
            ...
            for quantity, bundle, degree in zip(
                ...
                if bundle == "S":
                    rhs = rhs + SVLieDerivative(
                        degree,
                        dim,
                        dfdx_vars["F"] / total_dens,
                        qvars[quantity],
                        xhats[quantity],
                    )
                elif bundle == "VV":
                    rhs = rhs + VVLieDerivative(
                        degree,
                        dim,
                        dfdx_vars["F"] / total_dens,
                        qvars[quantity],
            ...
            for quantity, bundle, degree in zip(
                ...
                if bundle == "S":
                    rhs = rhs + SVLieDerivative(
                        degree, dim, dfdx_vars["u"], qvars[quantity], xhats[quantity]
                    )
                elif bundle == "VV":
                    rhs = rhs + VVLieDerivative(
                        degree, dim, dfdx_vars["u"], qvars[quantity], xhats[quantity]
...
class MetriplecticDynamics(Dynamics):
    ...
    def linear_rhs(self, const_state, xstar, xhats, terms="all"):
        ...
        if terms == "all" or "model" in terms:
            ...
            for bracket in self.poisson_brackets:
                linear_rhs = linear_rhs + bracket.linear_rhs(
                    const_state, dfdx_linear_vars, xhats
                )
                for metric_bracket in self.metric_brackets:
                    linear_rhs = linear_rhs + metric_bracket.linear_rhs(
                        const_state, dfdx_linear_vars, xhats
                    )
        for term in self.forcing_terms:
            if terms == "all" or term in terms:
...
def get_dynamics(parameters, mesh, spaces, logger, initcond):
    ...
    if parameters["model"] in ["tswe-lp", "ce-lp", "mtswe-lp"]:
        ...
        if parameters["model"] == "mtswe-lp":
            vars = MoistThermalShallowWaterVariables_LP(
                spaces, parameters["tracer_names"]
            )
            hamiltonian = MoistThermalShallowWater_Hamiltonian_LP(vars)
        ...
        poisson_brackets = [
            LiePoisson_AdvectedDensities_Bracket(spaces, vars, parameters),
        ]
        metric_brackets = [
            ThermodynamicallyCompatibleViscousRegularization_LP(spaces, vars),
        ]
        entropy = SimpleEntropy(spaces, vars)
        statistics = AdvDensStatistics_LP(
            spaces,
            hamiltonian,
            vars,
            parameters["num_steps"] // parameters["stat_freq"] + 1,
        )
        diagnostics = AdvDensDiagnostics_LP(
            spaces, hamiltonian, vars, parameters["dim"]
        )
        ...
        if parameters["model"] == "mtswe-cf":
            vars = MoistThermalShallowWaterVariables_CF(
                spaces, parameters["tracer_names"]
            )
            hamiltonian = MoistThermalShallowWater_Hamiltonian_CF(vars)
        if parameters["model"] == "ce-cf":
            ...
            hamiltonian.thermo.set_thermo_const(initcond)
        poisson_brackets = [
            CurlForm_AdvectedDensities_Bracket(spaces, vars, parameters),
        ]
        metric_brackets = [
            ThermodynamicallyCompatibleViscousRegularization_CF(spaces, vars),
        ]
        entropy = SimpleEntropy(spaces, vars)
        statistics = AdvDensStatistics_CF(
            spaces,
            hamiltonian,
            vars,
            parameters["num_steps"] // parameters["stat_freq"] + 1,
        )
        diagnostics = AdvDensDiagnostics_CF(
            spaces, hamiltonian, vars, parameters["dim"]
        )

    elif parameters["model"] in ["tswe-cf-h1", "mtswe-cf-h1"]:
        if parameters["model"] == "tswe-cf-h1":
            vars = ThermalShallowWaterVariables_CF_H1(
                spaces, parameters["tracer_names"], parameters["dg_tracer_names"]
            )
            hamiltonian = ThermalShallowWater_Hamiltonian_CF_H1(vars)
        if parameters["model"] == "mtswe-cf-h1":
            vars = MoistThermalShallowWaterVariables_CF_H1(
                spaces, parameters["tracer_names"], parameters["dg_tracer_names"]
            )
            hamiltonian = MoistThermalShallowWater_Hamiltonian_CF_H1(vars)
        poisson_brackets = [
            CurlForm_AdvectedDensities_Bracket_H1(spaces, vars, parameters),
        ]
        ...
        entropy = SimpleEntropy(spaces, vars)
        statistics = AdvDensStatistics_CF_H1(
            spaces,
            hamiltonian,
            vars,
            parameters["num_steps"] // parameters["stat_freq"] + 1,
        )
        diagnostics = AdvDensDiagnostics_CF_H1(
            spaces, hamiltonian, vars, parameters["dim"]
        )
        ...
        statistics = MHDStatistics(
            spaces, parameters["num_steps"] // parameters["stat_freq"] + 1
        )
        ...
        poisson_brackets = [
            LiePoisson_AdvectedDensities_Bracket(spaces, vars, parameters),
            MaxwellBracket(spaces, vars, parameters),
        ...
        statistics = EulerMaxwellStatistics(
            spaces, parameters["num_steps"] // parameters["stat_freq"] + 1
        )
        ...
        poisson_brackets = [
            ScalarWaveBracket(spaces),
        ]
        metric_brackets = []
        entropy = None
        hamiltonian = ScalarWave_Hamiltonian(spaces)
        statistics = ScalarWaveStatistics(
            spaces, parameters["num_steps"] // parameters["stat_freq"] + 1
        )

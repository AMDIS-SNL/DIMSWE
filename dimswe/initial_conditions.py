...
def get_initial_condition(parameters):
    if parameters["initialcondition"] == "gaussian":
        return Gaussian(parameters)
    elif parameters["initialcondition"] == "doublevortex":
        return DoubleVortex(parameters)
    elif parameters["initialcondition"] == "densitywave":
        return DensityWave(parameters)

    # ADD LOTS HERE!!!
    elif parameters["initialcondition"] == "RP1":
        return RiemannProblem1(parameters)
    elif parameters["initialcondition"] == "RP2":
        return RiemannProblem2(parameters)
...
class IC1D(IC):
    ...
    def gaussian_tracer(self, h, x, t):
        s = 0.1 * (
            ...
            + 0.05
            * exp(
                -((x[0] - self.xc) * (x[0] - self.xc))
                / (1.0 / 9 * 0.5 * 0.5 * self.Lx * self.Lx)
            )
        )
        return s * h

    def square_tracer(self, h, x, t):
        xscaled = (x[0] - self.xc + self.Lx / 2.0) / self.Lx
        return ufl.conditional(
            ufl.gt(xscaled, 2.0 / 3.0),
            0.0,
            ufl.conditional(ufl.lt(xscaled, 1.0 / 3.0), 0.0, 0.1 * h),
        )
    ...
    def set_tracers(self, x, t, initcond):
        for i, name in enumerate(self.parameters["tracer_names"]):
            if self.parameters["tracer_init_conds"][i] == "block":
                ...
                raise ValueError(
                    "unknown tracer initial condition "
                    + self.parameters["tracer_init_conds"][i]
                )
        ...
        for i, name in enumerate(self.parameters["dg_tracer_names"]):
            if self.parameters["dg_tracer_init_conds"][i] == "block":
                ...
                raise ValueError(
                    "unknown dg tracer initial condition "
                    + self.parameters["dg_tracer_init_conds"][i]
                )
...
class RiemannProblem(IC1D):
    ...
    def get_value(self, mesh, t):
        ...
        initcond = {}
        initcond["rho"] = ufl.conditional(
            ufl.le(xs[0], self.xc_discont), self.rhol, self.rhor
        )
        initcond["v"] = ufl.conditional(
            ufl.le(xs[0], self.xc_discont), self.ul, self.ur
        )
        p = ufl.conditional(ufl.lt(xs[0], self.xc_discont), self.pl, self.pr)
        eta = self.thermo.get_eta(initcond["rho"], p)
        initcond["m"] = ufl.conditional(
            ufl.le(xs[0], self.xc_discont), self.rhol * self.ul, self.rhor * self.ur
        )
...
class Gaussian(IC1D):
    ...
    def get_value(self, mesh, t):
        ...
        s = self.g * (
            ...
            + self.c
            * exp(
                -((xs[0] - self.xc) * (xs[0] - self.xc))
                / (self.a * self.a * self.D * self.D)
            )
        )
...
class LeBlanc(RiemannProblem):
    def __init__(self, params):
        ...
        self.rhol = 1.0
        self.ul = 0.0
        self.pl = (2.0 / 3.0) * 1.0e-1
        self.rhor = 1.0e-3
        self.ur = 0.0
        self.pr = (2.0 / 3.0) * 1.0e-10
    ...
    def gaussian_tracer(self, h, xs, t):
        s = 0.1 * (
            ...
            * exp(
                -(
                    (xs[0] - self.xc) * (xs[0] - self.xc)
                    + (xs[1] - self.yc) * (xs[1] - self.yc)
                )
                / (1.0 / 9.0 * 0.5 * 0.5 * self.Lx * self.Ly)
            )
        )
        return s * h

    def square_tracer(self, h, xs, t):
        xscaled = (xs[0] - self.xc + self.Lx / 2.0) / self.Lx
        yscaled = (xs[1] - self.yc + self.Ly / 2.0) / self.Ly
        return ufl.conditional(
            ufl.gt(xscaled, 2.0 / 3.0),
            0.0,
            ufl.conditional(
                ufl.lt(xscaled, 1.0 / 3.0),
                0.0,
                ufl.conditional(
                    ufl.gt(yscaled, 2.0 / 3.0),
                    0.0,
                    ufl.conditional(ufl.lt(yscaled, 1.0 / 3.0), 0.0, 0.1 * h),
                ),
            ),
        )
...
class DensityWave(IC2D):
    def __init__(self, params):
        ...
        self.const_state["u"] = 0.0

    def get_value(self, mesh, t):
        xs = SpatialCoordinate(mesh)

        initcond = {}
        initcond["h"] = 1.0 + 1 / (4.0 * pi) * self.coriolis / self.g * sin(
            4.0 * pi * xs[1]
        )
        u = 0.0
        v = sin(2.0 * pi * xs[0])
...
class DoubleVortex(IC2D):
    def __init__(self, params):
        ...
        self.Lx = 5000.0 * 1000.0
        self.Ly = 5000.0 * 1000.0
        self.xc = 0.5 * self.Lx
        self.yc = 0.5 * self.Ly
        self.g = 9.80616
        self.H0 = 750.0
        self.ox = 0.1  # 0.1
        self.oy = 0.1  # 0.1
        self.sigmax = 3.0 / 40.0 * self.Lx  # 3.
        self.sigmay = 3.0 / 40.0 * self.Lx  # 3.
        self.dh = 75.0
    ...
    def get_value(self, mesh, t):
        ...
        xprimeprime1 = (
            self.Lx
            / (2.0 * pi * self.sigmax)
            * sin(2 * pi / self.Lx * (xs[0] - self.xc1))
        )
        yprimeprime1 = (
            self.Ly
            / (2.0 * pi * self.sigmay)
            * sin(2 * pi / self.Ly * (xs[1] - self.yc1))
        )
        xprimeprime2 = (
            self.Lx
            / (2.0 * pi * self.sigmax)
            * sin(2 * pi / self.Lx * (xs[0] - self.xc2))
        )
        yprimeprime2 = (
            self.Ly
            / (2.0 * pi * self.sigmay)
            * sin(2 * pi / self.Ly * (xs[1] - self.yc2))
        )

        initcond = {}
        initcond["h"] = self.H0 - self.dh * (
            exp(-0.5 * (xprime1 * xprime1 + yprime1 * yprime1))
            + exp(-0.5 * (xprime2 * xprime2 + yprime2 * yprime2))
            - 4.0 * pi * self.sigmax * self.sigmay / self.Lx / self.Ly
        )
        u = (
            -self.g
            * self.dh
